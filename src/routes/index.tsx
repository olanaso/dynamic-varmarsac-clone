...
import aboutFleet from "@/assets/about-fleet.avif";
import truck from "@/assets/truck.avif";
import s1 from "@/assets/flota-1.avif";
import s2 from "@/assets/flota-2.avif";
import s3 from "@/assets/flota-3.avif";
import s4 from "@/assets/flota-4.avif";
import s5 from "@/assets/flota-5.avif";
import s6 from "@/assets/flota-6.avif";
import cond1 from "@/assets/cond-1.avif";
import mapCoverage from "@/assets/map-coverage.avif";
import cond2 from "@/assets/cond-2.avif";
import cond3 from "@/assets/cond-3.avif";
...
async function fetchHomeFleetVehicles(): Promise<PublicVehicle[]> {
  const base = resolveApiBase();
  const res = await fetch(`${base}/api/vehicles?page=1&limit=12`);

  if (!res.ok) {
    throw new Error("No se pudo cargar la flota pública.");
  }

  const json = (await res.json()) as { data?: PublicVehicle[] };
  ...
  head: () => ({
    meta: [
      { title: "VARMAR Contratistas Generales — Inicio" },
      {
        name: "description",
        content:
          "Construimos proyectos con calidad y confianza. Obras civiles, servicios generales y proyectos integrales.",
      },
    ],
  }),
  component: Index,
...
function Index() {
  ...
  const {
    data: homeFleetVehicles = [],
    isLoading: isHomeFleetLoading,
    isError: isHomeFleetError,
  } = useQuery({
    queryKey: ["home-fleet-vehicles"],
    queryFn: fetchHomeFleetVehicles,
    staleTime: 1000 * 60 * 5,
  });
    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 md:px-8 md:py-20 lg:grid-cols-2">
          <div>
            <h2 className="text-4xl font-extrabold leading-tight text-brand-blue-dark md:text-5xl lg:text-[3.25rem]">
              {t("home.aboutTitle1")}{" "}
              <span className="text-brand-blue">{t("home.aboutTitle2")}</span>
            </h2>
            ...
            <p className="mt-5 text-sm font-bold uppercase tracking-wider text-brand-blue-dark">
              "{t("home.aboutP2pre")}
              <span className="text-brand-red">{t("home.aboutP2bold")}</span>
              {t("home.aboutP2mid")}
              <span className="text-brand-red">{t("home.aboutP2bold2")}</span>"
            </p>
          </div>
        ...
        <div className="mx-auto grid max-w-7xl items-start gap-12 px-4 md:px-8 lg:grid-cols-2">
          <div>
            <h2 className="text-4xl font-extrabold text-brand-blue-dark md:text-5xl">
              ...
              <span className="text-brand-blue">{t("home.servicesTitle2")}</span>
            </h2>
      ...
      <section className="bg-slate-200 py-10 md:py-12">
        ...
        {isHomeFleetLoading ? (
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <div key={item} className="aspect-[4/3] animate-pulse rounded-sm bg-slate-300/70" />
            ))}
          </div>
        ) : marqueeFleet.length > 0 ? (
          <div
          ...
          >
            <div className="flex w-max animate-marquee gap-6 group-hover:[animation-play-state:paused]">
              {marqueeFleet.map((item, i) => (
                <div key={`${item.id}-${i}`} className="group/card w-[280px] shrink-0 sm:w-[340px]">
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map(([n, l]) => (
              <div
                key={l}
                className="rounded-2xl bg-white p-8 text-center shadow-[var(--shadow-card)] transition hover:-translate-y-1"
              >
      ...
      <section className="bg-muted py-20 lg:py-28">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 md:px-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-6 text-lg font-semibold text-foreground/80">
              Operamos en el sur del Perú
            </p>
            <p className="mt-5 text-lg text-muted-foreground">
              Con sedes estratégicas en Cusco y Apurímac, brindamos servicio a las principales
              operaciones mineras y destinos turísticos de la región.
            </p>
                ...
                <div className="relative">
                  ...
                  <div className="flex items-start gap-1.5">
                    ...
                    <span className="text-sm text-foreground/80">
                      Las Bambas — Barrio Manantiales S/N
                    </span>
                  </div>
                  <div className="mt-3">
      ...
      <section className="bg-brand-blue-dark py-16 md:py-20">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 md:grid-cols-2 md:px-8">
          <div className="flex flex-col items-center md:items-start">
            <h2 className="text-5xl font-extrabold uppercase tracking-tight text-white md:text-6xl">
              {t("home.valuesTitle")}
            </h2>
            ...
            <HandshakeIcon
              className="mt-8 h-40 w-40 text-white md:h-52 md:w-52"
              strokeWidth={1.2}
            />
          </div>
      ...
      <section className="py-16 md:py-20 bg-slate-100">
        <div className="relative mx-auto max-w-5xl px-4 text-center md:px-8">
          <h2 className="text-3xl font-extrabold md:text-5xl text-brand-blue-dark">
            {t("home.ctaTitle")}
          </h2>
          ...
          <button
            ...
            onClick={() =>
              openWhatsApp(
                "Hola, quiero solicitar una cotización para mi próximo proyecto con VARMAR Contratistas Generales.",
              )
            }
            className="mt-8 inline-flex cursor-pointer items-center gap-2 rounded-xl bg-brand-red px-8 py-4 text-sm font-bold text-white shadow-[var(--shadow-glow)] transition-all hover:-translate-y-0.5"
          >
    ...
    <section className="bg-brand-soft py-16 md:py-20">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-4xl font-extrabold text-brand-blue-dark md:text-5xl">Blog</h2>
            <p className="mt-3 text-base font-medium text-brand-blue-dark">
              Noticias y perspectivas para el{" "}
              <span className="font-extrabold text-brand-red">Perú de hoy y mañana</span>
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="mt-10 rounded-sm border border-brand-red/20 bg-white p-6 text-sm font-medium text-foreground/70 shadow-[var(--shadow-card)]">
            No pudimos cargar las publicaciones en este momento. Puedes visitar el blog desde el
            botón "Ver todo".
          </div>
        ) : posts.length > 0 ? (
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
